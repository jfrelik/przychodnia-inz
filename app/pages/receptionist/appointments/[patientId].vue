<script lang="ts" setup>
	...
	const patient = computed(
		() =>
			(directoryData.value ?? []).find(
				(user) => user.userId === patientId.value && user.role === 'user'
			) ?? null
	);
	...
	const visitTypeOptions = [
		{ label: 'Konsultacja (20 min)', value: 'consultation' as const },
		{ label: 'Procedura (60 min)', value: 'procedure' as const },
	];
	...
	const visitTypeLabel = computed(
		() =>
			visitTypeOptions.find((v) => v.value === visitType.value)?.label ??
			'Konsultacja (20 min)'
	);
	...
	const fetchSlots = async () => {
		...
		try {
			...
			const response = await $fetch<{
				...
				endDate: string;
				slots: SlotResult[];
			}>('/api/receptionist/availableSlots', {
				params: {
					startDate: start,
					endDate: end,
	...
	const bookVisit = async () => {
		...
		try {
			await $fetch('/api/receptionist/appointments', {
				method: 'POST',
				body: {
	...
	const backToDirectory = () => {
		router.push('/receptionist/directory');
	};
</script>
						...
						<div class="flex w-full flex-col items-center gap-3">
							...
							>
								<USelect
									...
									class="w-full cursor-pointer"
									:disabled="!patient || visitType === 'procedure'"
								/>
							</UFormField>
							<p class="text-sm text-neutral-600">
								Rodzaj: {{ visitTypeLabel }}. Procedura rezerwuje 60 minut.
							</p>
						</div>
						<div class="flex flex-row justify-between gap-3">
							<UButton
						...
						<div class="flex flex-col gap-4">
							...
							<div v-if="!slotsPending && slots.length" class="space-y-2">
								<p class="text-sm font-semibold text-gray-700">
									{{
										selectedDay ? formatDateLabel(selectedDay) : 'Wybierz dzien'
									}}
								</p>
